...
PyObject *Cs_disasm(PyObject *self, PyObject *args)
{
    ...
    count = cs_disasm(((Cs *)self)->handle, (const uint8_t *)buffer, buflen,
                      addr, 0, &insn);
    ...
    for (j = 0; j < count; j++) {
        ...
        memcpy((char *)py_insn + offsetof(CsInsn, insn_), &insn[j],
               sizeof(cs_insn));
...
PyObject *Cs_disasm_lite(PyObject *self, PyObject *args)
{
    ...
    for (j = 0; j < count; j++) {
        tuple = Py_BuildValue("(L, H, s, s)", insn[j].address, insn[j].size,
...
PyMethodDef Cs_methods[] = {
    ...
    {"disasm_lite", (PyCFunction)Cs_disasm_lite, METH_VARARGS,
     "disassemble lite"},
    {NULL},
};
